...
- **Modern Tooling**: Bun package manager, Vite bundler, ESLint for code quality
- **API Integration**: Frontend service layers that consume external APIs (TMDB Movie API)
- **State Management**: Redux Toolkit with Redux Saga for state management and async side effects
...
- **API Integration Focus**: Treat backend APIs as external services to be consumed (TMDB Movie API)
- **Frontend Service Layers**: Build robust frontend service layers using Redux Saga for async operations
- **Client-Side Logic**: Implement business logic on the frontend with Redux Toolkit slices and sagas
- **Modern React Patterns**: Use functional components, hooks, and modern React patterns
- **Performance Optimization**: Optimize for frontend performance using React best practices and memoization
...
- `bun run lint`: Code quality verification with ESLint
- `bun run build`: Production build verification (TypeScript + Vite)
- `bun run dev`: Development server for testing changes
...
- **Consistent patterns**: Use consistent coding patterns across all domains and modules
- **Modern tooling**: Leverage Bun, Vite, and ESLint effectively
- **Type safety**: Maintain strict TypeScript usage across all components, slices, and sagas
- **Performance monitoring**: Monitor bundle sizes and loading performance
- **Redux patterns**: Follow Redux Toolkit and Redux Saga best practices for predictable state management

**Quality Assurance Process:**

1. **Code review**: All changes reviewed for quality and architectural alignment
2. **Linting**: ESLint enforcement for code quality and consistency
3. **Performance testing**: Regular performance audits for bundle size and runtime
4. **Type checking**: Strict TypeScript enforcement across the codebase (verified via build)
